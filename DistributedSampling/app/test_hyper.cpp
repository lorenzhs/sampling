...
int main(int argn, char **argv) {
    ...
    VarGen hyp(config.seed);
    StochasticLib1 stocc(config.seed);
    std::cout << "warmup" << std::endl;
    for (ULONG iteration = 0; iteration < std::min((ULONG)100, config.iterations); ++iteration) {
        std::cout << hyp.Hypergeometric(config.n, config.N/2, config.N) << std::endl;
        std::cout << stocc.Hypergeometric(config.n, config.N/2, config.N) << std::endl;
    }
