 ...
 */
template <typename resolution = std::chrono::microseconds, int scaling_factor = 1000, typename return_type = double>
struct base_timer {
    base_timer() : start() {
        reset();
    }
    ...
    return_type get() const {
        auto duration = std::chrono::duration_cast<resolution>(std::chrono::system_clock::now() - start);
